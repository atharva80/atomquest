...
export function GoalChatbot() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      {/* Floating Chat Panel */}
      {isOpen && (
        <Card className="mb-4 w-[350px] sm:w-[400px] h-[500px] shadow-2xl flex flex-col overflow-hidden border border-indigo-100 dark:border-indigo-950 bg-white dark:bg-slate-950 rounded-2xl animate-in fade-in slide-in-from-bottom-6 duration-300">
          
          {/* Header */}
          <CardHeader className="bg-gradient-to-r from-indigo-600 via-indigo-700 to-violet-700 text-white p-4 flex flex-row items-center justify-between space-y-0 shadow-md">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-white/10 rounded-xl relative">
                <Bot className="w-5 h-5" />
                <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-400 border-2 border-indigo-700 rounded-full animate-pulse" />
              </div>
              <div>
                <CardTitle className="text-sm font-semibold tracking-wide flex items-center gap-1.5 font-heading">
                  Goal Coach <Sparkles className="w-3.5 h-3.5 text-amber-300 animate-pulse" />
                </CardTitle>
                <span className="text-[10px] text-indigo-100 font-medium font-sans">Active & Ready</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1">
              <Button 
                ...
                className="h-8 w-8 text-indigo-100 hover:text-white hover:bg-white/10 rounded-full transition-all"
            ...
            <div className="px-4 py-2 border-t bg-slate-50/50 dark:bg-slate-950/20 flex flex-wrap gap-1.5 shrink-0">
              {SUGGESTIONS.map((sug, i) => (
                <button
                  ...
                  className="text-[11px] font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/30 dark:hover:bg-indigo-950/60 border border-indigo-100/50 dark:border-indigo-900/30 rounded-full px-3 py-1 text-left transition-all duration-200 active:scale-95 shadow-sm"
                >
                  {sug}
                </button>
              ))}
          ...
          <div className="p-3 border-t bg-slate-50/50 dark:bg-slate-950/30 flex gap-2 items-center shrink-0">
            <Input
              ...
              disabled={isLoading}
              className="flex-1 text-sm bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 rounded-xl focus-visible:ring-indigo-500/30 focus-visible:ring-offset-0 focus-visible:border-indigo-500 h-10 shadow-inner"
            />
            <Button
              size="icon"
              onClick={() => handleSend(input)}
              disabled={isLoading || !input.trim()}
              className="h-10 w-10 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md transition-all shrink-0 active:scale-95"
            >
      ...
      <button
        ...
        className={`p-4 bg-gradient-to-r from-indigo-600 via-indigo-700 to-violet-700 hover:from-indigo-500 hover:to-violet-600 text-white rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 transform active:scale-95 group relative ${
          isOpen ? 'rotate-90 bg-slate-800 hover:bg-slate-900' : 'hover:scale-105'
        }`}
      >
        {/* Glow Ring ring animation */}
        <span className="absolute inset-0 rounded-full border border-indigo-400/30 dark:border-indigo-400/20 group-hover:animate-ping opacity-60" />
