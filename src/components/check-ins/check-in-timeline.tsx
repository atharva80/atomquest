  ...
  return (
    <div className="relative pl-6 space-y-8 before:absolute before:inset-0 before:ml-8 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 dark:before:via-slate-800 before:to-transparent">
      {quarters.map((q, index) => {
        ...
        let score = 0;
        if (checkin) {
          score = calculateProgressScore(goal.uom_type, goal.target || 0, checkin.actual_achievement);
        }
              ...
              <div className={cn(
                ...
                <div className="flex justify-between items-start mb-2">
                  <div className="font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                    {q} Update
                    {checkin && <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">{checkin.progress_status.replace('_', ' ')}</span>}
                  </div>
                ...
                {checkin ? (
                  <div className="space-y-3">
                    <div className="text-sm flex items-center justify-between bg-slate-50 dark:bg-slate-800/50 p-2 rounded">
                      ...
                      <span className="font-medium text-slate-900 dark:text-white">{checkin.actual_achievement}</span>
                    </div>
